...
**每月操作**：把三个源文件（原始记录.xlsx、考勤表.xlsx、单据.pdf）放到 `inputfile/` 目录，运行 `python main.py` 即可。**支持随意命名**，程序通过文件名关键词 + 文件内容特征自动识别。

输出：`output/考勤对账差异报告_<周期开始>_<周期结束>.xlsx`

## 自动检测

`config.json` 中 `files` 和 `period` 留空时，程序自动扫描 `inputfile/` 目录：

- **原始记录**：文件名含"原始记录/打卡"，或内容含打卡时间列
- **考勤表**：文件名含"考勤"，或内容含"上/下/加/夜間工作"行标签
- **PDF**：目录下最新的 pdf 文件
- **考勤周期**：从文件名提取（如 `20260721-20260820`），或从原始记录的打卡时间列自动推断
